# Take-Home Assignment — Full-Stack Developer (2–3 yrs)
...
| **Deadline** | **2 calendar days** from the moment this email lands in your inbox. No start-date calendar — your clock starts the instant you receive it. |
| **Effort target** | 8–12 focused hours. Quality > quantity. Do not burn a whole weekend. |
| **Submit to** | `careers@exhibit.co.in` — subject: `MiniInfluencer Assignment — <Your Full Name>` |
...
## 2. What you are building — "MiniInfluencer"
...
**Recommended starter:** `laravel new miniinfluencer --react --typescript` (the official Laravel React Starter Kit).
...
|---|---|---|
| **Apify** | $5/month free credit (plenty for this task) | [apify.com](https://apify.com) → use the public "Instagram Profile Scraper" actor |
| **RapidAPI** | Free tier on several IG scrapers (e.g. `instagram-scraper-api2`, `instagram-looter2`) | [rapidapi.com](https://rapidapi.com) |
...
**Quick primer on token bucket:** imagine a bucket that holds 100 tokens. Each API call removes 1. The bucket refills at 10/min. If empty, the job waits. [5-min video](https://www.youtube.com/results?search_query=token+bucket+algorithm) — search "token bucket algorithm" if you're new to this.
...
- All timestamps use **`timestamptz`** (timestamp WITH time zone). Never `timestamp`.
...
Writing a snapshot row AND updating the parent profile (`last_refreshed_at`, `followers_count`) must happen in **one DB transaction**. If the worker crashes mid-write, the data must not split.

**Time-series query:**
"Show me the last 30 days of snapshots for this profile" must be efficient (uses an index). Show the query and plan in the README.
...
Email `careers@exhibit.co.in` with subject `MiniInfluencer Assignment — <Your Full Name>` containing:
...
- Any `timestamp` column (must be `timestamptz`).
